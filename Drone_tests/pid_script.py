...
class TunerControlCF:
    ...
    def send_pid_gains(self):
        print("Sending to the " + self.axis_choice + "position PID controller: Kp: "
            + str(self.pid_gui.scale_Kp.get()) + ", Ki: " + str(self.pid_gui.scale_Ki.get()) + ", Kd: "+str(self.pid_gui.scale_Kd.get()))
        ...
        with SyncLogger(self.cf, log_config) as logger:
            for log_entry in logger:
                ...
                if ((time.time() - current_time) > STEP_RESPONSE_TIME):
                    break
        print(f'Time: {time_history}    :     Position: {pos_history}')
        #print(sp_history)
...
if __name__ == '__main__':
    ...
    lg_stab = LogConfig(name='Motor_ints', period_in_ms=10)
